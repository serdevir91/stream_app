...
async function loadSearchResult(idx, silent) {
    ...
    try {
        ...
        // Deduplicate: if already loaded, just activate
        const existingIdx = state.subtitles.findIndex(s => 
            s.label === subFileName || 
        ...
        if (existingIdx >= 0) {
            ...
            // Build wyzie URL from the OpenSubtitles download link.
            // Pattern: https://dl.opensubtitles.org/.../vrf-<vrf>/filead/<id>.gz
            // Wyzie:   https://sub.wyzie.io/c/<vrf>/id/<id>?format=vtt&encoding=UTF-8
            const wyzieUrl = (function() {
                var m = sub.SubDownloadLink.match(/\/vrf-([a-f0-9]+)\/filead\/(\d+)/i);
                if (!m) return null;
                return 'https://sub.wyzie.io/c/' + m[1] + '/id/' + m[2] + '?format=vtt&encoding=UTF-8';
            })();
            
            let cues, cachedVttUrl = null;
            
            if (wyzieUrl) {
                // Fetch VTT text from wyzie for cue parsing (needed for the custom overlay).
                // The wyzie URL is also passed as cachedVttUrl so the <track> element and
                // iOS/Chromecast renderers point directly at it — no data URL round-trip.
                _log('Subtitle: fetching VTT from wyzie', wyzieUrl);
                const wyzieRes = await fetch(wyzieUrl);
                if (!wyzieRes.ok) throw new Error('wyzie fetch failed: ' + wyzieRes.status);
                const vttText = await wyzieRes.text();
                if (!vttText || vttText.indexOf('-->') === -1) throw new Error('Invalid VTT from wyzie');
                cues = parseVTT(vttText); // wyzie returns VTT — no srtToVtt needed
                cachedVttUrl = wyzieUrl;
            } else {
                // Fallback: download gzip from OpenSubtitles, decompress, parse
                _log('Subtitle: no wyzie URL, downloading gzip from source');
                const response = await fetch(sub.SubDownloadLink);
                ...
                if (!content || content.indexOf('-->') === -1) throw new Error('Invalid subtitle format');
                const vtt = content.trimStart().startsWith('WEBVTT') ? content : srtToVtt(content);
                cues = parseVTT(vtt);
            }
